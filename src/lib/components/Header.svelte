<script>
	...
	$: if ($textToSearch && $textToSearch.trim.length > 0) expandedSearchForm = true;
</script>

<div>
	<header class="header">
		<!--
		<a href="/" class="logo"> <i class="fas fa-shopping-basket" /> IMMER</a>
		-->
		<a href="/" class="logo">
		...
		<div class="icons">
			<div id="menu-btn" class="fas fa-bars" on:click|preventDefault={showMenu} />
			<div
				id="search-btn"
			...
			{#if isAuth}
				...
				<div id="login-btn" class="fas fa-user" on:click|preventDefault={showLoginForm} />
			{:else}
				<div
					id="login-btn"
		...
		{#if expandedLoginForm}
			{#if isAuth}
				<UserNavbar on:hideForm={hideLoginForm} showLoginForm={() => showLoginForm()} />
			{:else}
				<Login on:hideForm={hideLoginForm} showLoginForm={() => showLoginForm()} />
			{/if}
		{/if}
		{#if expandedCartForm}
