<script context="module" lang="ts">
	...
	export async function load({ fetch, params }) {
		...
		states = await resStates.objectList;

		const resCities = await (
			await fetch('../api/getCities', {
				headers: {
					'Content-Type': 'application/json'
				},
				method: 'POST',
		...
		cities = await resCities.objectList;

		const resDistricts = await (
			await fetch('../api/getDistricts', {
				headers: {
					'Content-Type': 'application/json'
				},
				method: 'POST',
		...
		districts = await resDistricts.objectList;
		return {
			props: {}
		};
	}
...
<script lang="ts">
	async function addAddress(personId) {
		const districtBean: District = {
			districtId: 0,
			name: '',
			country: undefined,
			region: undefined,
			state: { stateId: 194, name: '', country: undefined, region: undefined },
			city: { cityId: 496, name: '', country: undefined, region: undefined, state: undefined }
		};
		...
		if (responseJson.status != '200') {
			...
			personAddress.personAddressId = responseJson.objectList[0]!.personAddressId;
	...
	async function updateAddress(id) {
		const response = await fetch('../api/addresses', {
			...
			method: 'PUT',
			body: JSON.stringify({
				personAddress: $currentUser.person.addresses.find(
	...
	function resetDistric(address: PersonAddress) {
		let userDistrict = null;
		const filteredDistricts = getDistricts(address.disctrictBean.city.cityId);
		if (filteredDistricts.length > 0) {
			userDistrict = filteredDistricts[0].districtId;
			address.disctrictBean.districtId = userDistrict;
		}
	...
	function getCities(stateId: number) {
		const filteredCities = cities.filter((o) => o.state.stateId === stateId);
		if (filteredCities) return filteredCities;
		else return [];
	}

	function getDistricts(cityId: number) {
		const filteredDistricts = districts.filter((o) => o.city.cityId === cityId);
...
<div class="heading">
	<h1>Mi Perfilssss</h1>
	<p><a href="/">Inicio >></a> Mis Direcciones</p>
</div>
						...
						<tbody>
								...
								<td colspan="2">
									<div class="icons">
										<div
											id="login-btn"
											class="fas fa-save"
											on:click|preventDefault={() => updateAddress(address.personAddressId)}
										/>
										<div
											id="login-btn"
											class="fas fa-times"
											on:click|preventDefault={() => removeAddress(address.personAddressId)}
										/>
									</div>
								</td>
							</tr>
							<tr>
								<td>
									...
									<select
										...
										name="select-state"
										id="select-state"
										bind:value={address.disctrictBean.state.stateId}
										on:change={() => resetCity(address)}
									>
										<option disabled>--Seleccionar Provincia--</option>
								...
								<td>
									...
									<select
										...
										name="select-city"
										id="select-city"
										bind:value={address.disctrictBean.city.cityId}
								...
								<td>
									...
									<select
										...
										id="select-district"
										bind:value={address.disctrictBean.districtId}
									>
										<option disabled>--Seleccionar Parroquia--</option>
							...
							<tr>
								<td
									...
									<input type="text" class="full-width" bind:value={address.mainStreet} />
								</td>
								<td>
									<h2>Número de Casa</h2>
									<input type="text" class="full-width" bind:value={address.propertyNumber} />
								</td>

								<td>
									<h2>Calle Secundaria</h2>
									<input type="text" class="full-width" bind:value={address.secundaryStreet} />
								</td>
							</tr>
						</tbody>
					</table>
